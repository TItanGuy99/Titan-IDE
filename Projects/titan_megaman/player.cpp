/*Code and engine made by Titan Game Studios 2016/2021 coded by Luiz Nai.*/ #include "player.h"
...
void player::move(const std::vector < std::vector < int > > & map) {
  ...
  for (int i = 0; i < map.size(); i++)
    for (int j = start; j < end; j++) {
      ...
      SDL_Rect destrect = {
        j * player_size - baseclass::coord.x + player_size - 20,
        i * player_size - baseclass::coord.y,
        player_size / 2,
        player_size
      };
